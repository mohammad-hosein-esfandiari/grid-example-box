...
function lastFuncWinner(arr){
  ...
  const onDropFunc = (event) => {
    if (event.target.id) {
      ...
      setArr1(newArr);
      if (draggedColor == "black") {
        setBlack((prev) => prev - 1);
      }
      if (draggedColor == "white") {
        setWhite((prev) => prev - 1);
      }
      const findedNut = newArr.find(item=> item.id == event.target.id);
    const aaa = [check1(findedNut,newArr),check2(findedNut,newArr),check3(findedNut,newArr),check4(findedNut,newArr)]
      ...
      <div className="w-[400px] rounded-lg overflow-hidden bg-2 flex flex-wrap">
        {arr1.map((item,index) => (
          <div
            ...
            id={item.id}
            key={item.id}
            className={` flex h-[50px] w-[50px] ${index+1<57 && "border-b-[1px]" } ${(index+1) % 8 !== 0 && "border-r-[1px]" }   border-[#643f1867] items-center justify-center`}
         >
            {item.nut && (
...
<div className=" bg-white flex h-fit ml-4 bg-2 rounded-lg">
      ...
      <div className="  text-center pt-3 text-black font-bold text-[15px]  pr-4 ">
        <div className="flex items-center"><span className="text-[8px] mr-2 font-normal">BLACK nut count :</span> {black}</div>
        <div className="mt-5 pb-2 flex items-center "><span className="text-[8px] mr-2 font-normal">WHITE nut count :</span> {white}</div>
      </div>
</div>
    </div>
    </section>
